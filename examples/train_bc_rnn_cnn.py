...
def robosuite_hyperparameters(config):
    """
    Sets robosuite-specific hyperparameters for VISUAL training.
    """
    ...
    config.experiment.rollout.n = 20           # 减少每次rollout的次数，视觉测试很慢
    config.experiment.rollout.horizon = 400
    config.experiment.rollout.rate = 50        # 每50个epoch考一次试
    config.experiment.rollout.warmstart = 0
    config.experiment.rollout.terminate_on_success = True

    ## dataset loader config ##
    # 【关键修改 1】开启多线程读图，否则 GPU 会饿死
    config.train.num_data_workers = 2          

    # 【关键修改 2】千万别用 "all"，否则几十GB图片塞爆内存。改为只缓存低维数据
    ...
    # 【关键修改 3】减小 Batch Size！视觉模型极度吃显存，如果 OOM 可以改成 16 甚至 8
    config.train.batch_size = 32               
    config.train.num_epochs = 2000
    config.train.seed = 1

    ### Observation Config ###
    # 【关键修改 4】删除 "object" 键！让模型彻底变成闭卷考试，只能靠眼睛看！
    ...
    # 视觉特征提取器配置 (默认的 ResNet18 + SpatialSoftmax 组合非常经典)
    ...
    # 【关键修改 6】增加了隐层维度，以容纳更庞大的视觉特征 (ResNet 会吐出很多关键点)
