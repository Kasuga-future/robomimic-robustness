...
def _set_object_visual_rgba(mujoco_object, rgba):
    """
    Overwrite visual geom appearance with a flat RGBA color.

    This is useful for robust test environments where we want a controlled
    color shift without depending on MuJoCo material randomization behavior.
    """
    rgba_str = " ".join(str(v) for v in rgba)
    for geom in mujoco_object._obj.findall(".//geom"):
        if geom.get("group") == "1":
...
def _load_square_nut_variant_model(
    ...
    square_nut_rgba=None,
    square_sampler_x_range=(-0.115, -0.11),
    square_sampler_y_range=(0.11, 0.225),
    round_sampler_x_range=(-0.115, -0.11),
    round_sampler_y_range=(-0.225, -0.11),
    table_material=None,
    agentview_camera=None,
):
    """
    Shared loader for explicit NutAssemblySquare robustness variants.

    Variants are registered as separate robosuite environments so a policy
    checkpoint can be evaluated under different test-time shifts without any
    retraining or config rewriting.
    """
    super(NutAssembly, env)._load_model()
    ...
    if env.placement_initializer is None:
        env.placement_initializer = SequentialCompositeSampler(name="ObjectSampler")
        sampler_specs = [
            ("SquareNutSampler", square_sampler_x_range, square_sampler_y_range),
            ("RoundNutSampler", round_sampler_x_range, round_sampler_y_range),
        ]
        for sampler_name, x_range, y_range in sampler_specs:
            env.placement_initializer.append_sampler(
    ...
    for nut_cls, nut_name in zip((SquareNutObject, RoundNutObject), nut_names):
        nut = nut_cls(name=nut_name)
        if nut_name == "SquareNut":
            if square_nut_scale != 1.0:
                nut.set_scale(square_nut_scale)
            if square_nut_rgba is not None:
                _set_object_visual_rgba(nut, square_nut_rgba)
        env.nuts.append(nut)
        if isinstance(env.placement_initializer, SequentialCompositeSampler):
            env.placement_initializer.add_objects_to_sampler(
                sampler_name=f"{nut_name}Sampler",
                mujoco_objects=nut,
            )
        else:
            env.placement_initializer.add_objects(nut)

    env.model = ManipulationTask(
        mujoco_arena=mujoco_arena,
        mujoco_robots=[robot.robot_model for robot in env.robots],
        mujoco_objects=env.nuts,
    )
...
class NutAssembly(ManipulationEnv):
    ...
    def on_peg(self, obj_pos, peg_id):
        peg_success_xy_tol = getattr(self, "peg_success_xy_tol", 0.03)
        peg_success_z_margin = getattr(self, "peg_success_z_margin", 0.05)
        if peg_id == 0:
        ...
        res = False
        if (
            abs(obj_pos[0] - peg_pos[0]) < peg_success_xy_tol
            and abs(obj_pos[1] - peg_pos[1]) < peg_success_xy_tol
            and obj_pos[2] < self.table_offset[2] + peg_success_z_margin
        ):
            res = True
        return res
    ...
    def _check_success(self):
        ...
        for i, nut in enumerate(self.nuts):
            ...
            peg_success_reach_gate = getattr(self, "peg_success_reach_gate", 0.6)
            self.objects_on_pegs[i] = int(self.on_peg(obj_pos, i) and r_reach < peg_success_reach_gate)
...
class NutAssemblySquareObjectPerturb(NutAssemblySquare):
    """
    Square-task geometry OOD variant.

    The square nut is uniformly scaled down so the insertion geometry differs
    from the clean training distribution while the scene appearance and
    ...
    def _load_model(self):
        _load_square_nut_variant_model(
            env=self,
            square_nut_scale=0.90,
        )


class NutAssemblySquareColorPerturb(NutAssemblySquare):
    """
    Square-task color / texture OOD variant.

    The square nut switches to a flat blue appearance and the tabletop material
    is changed, while geometry and dynamics are preserved.
    """

    def _load_model(self):
        _load_square_nut_variant_model(
            env=self,
            square_nut_rgba=[0.08, 0.24, 0.95, 1.0],
            table_material="table_mat",
        )


class NutAssemblySquareCameraPerturb(NutAssemblySquare):
    """
    Square-task camera OOD variant.

    Only the external agentview camera pose and field of view are changed.
    The wrist camera remains untouched.
    """
    ...
    def _load_model(self):
        _load_square_nut_variant_model(
            env=self,
            agentview_camera={
                "pos": np.array([0.54, -0.12, 1.40]),
                "quat": np.array([0.641, 0.299, 0.299, 0.641]),
                "attribs": {"fovy": "72"},
            },
        )


class NutAssemblySquareVisualOOD(NutAssemblySquare):
    """
    Square-task visual-only OOD variant.

    Nut geometry, placement ranges, friction, and success logic are preserved.
    Only the appearance and the external camera are modified.
    """

    def _load_model(self):
        _load_square_nut_variant_model(
            env=self,
            square_nut_rgba=[0.10, 0.22, 0.95, 1.0],
            table_material="table_mat",
            agentview_camera={
                "pos": np.array([0.52, -0.10, 1.38]),
...
class NutAssemblySquareTaskDynamicsHard(NutAssemblySquare):
    """
    Square-task harder task / dynamics variant.

    Visual appearance remains close to the clean setup, while the initial nut
    distribution is widened, the table becomes more slippery, robot
    initialization noise is added, and the success tolerance is tightened.
    """
    ...
    def __init__(self, **kwargs):
        ...
        self.peg_success_xy_tol = 0.025
        self.peg_success_z_margin = 0.04
        self.peg_success_reach_gate = 0.5
        super().__init__(**kwargs)

    def _load_model(self):
        _load_square_nut_variant_model(
            env=self,
            square_sampler_x_range=(-0.145, -0.085),
            square_sampler_y_range=(0.08, 0.255),
        )
